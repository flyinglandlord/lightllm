...
class LlamaTransformerLayerInfer(TransformerLayerInferTpl):
    ...
    def _get_qkv(
        ...
        if getattr(infer_state, "use_vec_db", False) and infer_state.prefill_req_num > 0 and infer_state.prefill_b_split_ready_cache_len[0] >= 4096:
            ...
            _, I = infer_state.vec_db.index[0][self.layer_num_].search(np.ascontiguousarray(q_numpy[::32]), 4096)
            I = np.unique(I)
            if I[0] == -1: infer_state.past_kv_index = torch.tensor(I[1:], dtype=torch.int32, device="cuda")
            else: infer_state.past_kv_index = torch.tensor(I, dtype=torch.int32, device="cuda")

            torch.cuda.synchronize()
            kv = infer_state.mem_manager.kv_buffer[self.layer_num_]
            print(max(infer_state.past_kv_index), min(infer_state.past_kv_index))
    ...
    def _vec_db_context_attention(self, q, infer_state: SplitFuseInferStateInfo, layer_weight, o_tensor):
        ...
        if infer_state.prefill_req_num > 0:
            ...
            new_req_to_token_indexs = torch.linspace(0, total_length, total_length, dtype=torch.int32, device="cuda").unsqueeze(0)
            new_prefill_b_split_ready_cache_len = torch.tensor([len(infer_state.past_kv_index)], dtype=torch.int32, device="cuda")
