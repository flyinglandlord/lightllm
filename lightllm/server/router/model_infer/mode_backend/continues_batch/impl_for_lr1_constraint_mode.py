...
class LR1GrammarConstraintBackend(ContinuesBatchBackend):
    ...
    def prefill_batch(self, batch_id):
        ...
        for _, req_ids in req_dict.items():
            ...
            self._batched_mask_req_out_token(i_list, run_req_list, mask, batch.batch_lr1_stack, batch.batch_lr1_stack_size, prefill=True)
    ...
    def decode_batch(self, batch_id):
        ...
        for _, req_ids in req_dict.items():
            ...
            i_list = req_ids
            run_req_list = [run_reqs[i] for i in req_ids]
            self._batched_mask_req_out_token(i_list, run_req_list, mask, batch.batch_lr1_stack, batch.batch_lr1_stack_size, prefill=False)
    ...
    def _handle_req_ans(self, req_obj: InferReq, next_token_id, next_token_logprob, output_dict, batch, idx):
        ...
        if req_obj.sampling_param.lr1_grammar is not None and next_token_id not in self.eos_token_ids:
            ...
            batch.batch_lr1_stack[idx, :len(req_obj.sampling_param.lr1_stack)] = torch.tensor(req_obj.sampling_param.lr1_stack, dtype=torch.int32, device="cuda")
    ...
    def _batched_mask_req_out_token(self, i_list, run_obj_list, mask, lr1_stack=None, lr1_stack_size=None, prefill=False):
        #st = time.time()
        ...
        for sample_params in sample_params_list:
            ...
            # current_state_list.append(torch.tensor([sample_params.lr1_current_node_id] * vocab_size, dtype=torch.int32, device="cuda"))
