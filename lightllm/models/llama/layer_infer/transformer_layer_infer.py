...
class LlamaTransformerLayerInfer(TransformerLayerInferTpl):
    ...
    @calculate_time(show=False, min_cost_ms=0)
    def _get_qkv(
        ...
        if (
            getattr(infer_state, "use_vec_db", False)
            and infer_state.prefill_req_num > 0
            and infer_state.prefill_b_split_ready_cache_len[0] >= 4096
        ):
            q_numpy = q.view(-1, self.tp_q_head_num_ * self.head_dim_).cpu().numpy().astype(np.float32)
            _, Index = infer_state.vec_db.index[0][self.layer_num_].search(np.ascontiguousarray(q_numpy[::32]), 4096)
            Index = np.unique(Index)
            if Index[0] == -1:
                infer_state.past_kv_index = torch.tensor(Index[1:], dtype=torch.int32, device="cuda")
            else:
                infer_state.past_kv_index = torch.tensor(Index, dtype=torch.int32, device="cuda")
            kv = infer_state.mem_manager.kv_buffer[self.layer_num_]
            infer_state.kv_workspace = torch.cat([kv[infer_state.past_kv_index, :, :], cache_kv], dim=0)
            # print(infer_state.kv_workspace, infer_state.kv_workspace.shape)
            # print(infer_state.past_kv_index.shape)
    ...
    def _vec_db_context_attention(self, q, infer_state: SplitFuseInferStateInfo, layer_weight, o_tensor):
        ...
        if infer_state.prefill_req_num > 0:
            ...
            total_length = len(infer_state.past_kv_index) + split_size
            new_req_to_token_indexs = torch.linspace(
                0, total_length, total_length, dtype=torch.int32, device="cuda"
            ).unsqueeze(0)
            new_prefill_b_split_ready_cache_len = torch.tensor(
                [len(infer_state.past_kv_index)], dtype=torch.int32, device="cuda"
            )
            nwe_prefill_b_seq_len = torch.tensor([total_length], dtype=torch.int32, device="cuda")

            splitfuse_context_attention_fwd(
                q[infer_state.decode_req_num :, :].view(calcu_shape1),
                infer_state.kv_workspace[:, 0 : self.tp_k_head_num_, :],
                infer_state.kv_workspace[:, self.tp_k_head_num_ : self.tp_k_head_num_ + self.tp_v_head_num_, :],
                ...
                new_prefill_b_split_ready_cache_len,
                nwe_prefill_b_seq_len,
    ...
    def _splitfuse_attention_kernel(
        ...
        if (
            ...
            self._vec_db_context_attention(q, infer_state, layer_weight, o_tensor)
            return o_tensor
