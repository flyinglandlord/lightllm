...
TORCH_LIBRARY_EXPAND(CONCAT(TORCH_EXTENSION_NAME, _custom_ar), custom_ar) {
  ...
  custom_ar.def(
      "init_custom_gather_ar(int[] ipc_tensors, Tensor rank_data, "
      "int rank, bool full_nvlink) -> int");
  custom_ar.impl("init_custom_gather_ar", torch::kCUDA, &init_custom_gather_ar);
  custom_ar.def(
      "all_gather(int fa, Tensor inp, Tensor! out, int reg_buffer, "
      "int reg_buffer_sz_bytes) -> ()");
  custom_ar.impl("all_gather", torch::kCUDA, &all_gather);

  custom_ar.def("allgather_dispose", &allgather_dispose);
  custom_ar.def("allgather_register_buffer", &register_buffer);
  custom_ar.def("allgather_get_graph_buffer_ipc_meta", &get_graph_buffer_ipc_meta);
  custom_ar.def("allgather_register_graph_buffers", &register_graph_buffers);
